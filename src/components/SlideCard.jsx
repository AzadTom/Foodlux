...
const SlideCard = ({ item ,curr ,prev,next}) => {
    ...
    return (
        <section className='flex-none w-full flex justify-center items-center transition-transform ease-out duration-200 ' style={{transform:`translateX(-${curr*100}%)`}}>
            <div className=' w-full flex-col sm:flex-row  flex justify-center items-center bg-[var(--neutralblack)] border border-[var(--secondarycolor)]  relative rounded-2xl'>

                <div className="w-full flex-1 ">
                    <img src={item} alt="heroImg" className='w-full h-[320px]   bg-cover bg-center object-cover rounded-tl-2xl rounded-bl-2xl' />
                </div>
